...
module lacerta_tb;
  ...
  // RTL paths
  `define TOP_PATH uut.chip_core.mprj
  ...
  /*initial begin
	...
	reg [7:0] tb_sram [ACTIVE_SCREEN_WIDTH*SCREEN_HEIGHT];
	initial begin
  ...
  // this task can be used to send a byte to lacerta through host uart interface
	...
	function check_sram_consistency();
		for(int i = 0; i < ACTIVE_SCREEN_WIDTH*SCREEN_HEIGHT; i = i + 1) begin
			assert(tb_sram[i] === CY7C1049GN_i.mem[i]) else $error("ERROR: MEMORY MISMATCH, tb %2H, RTL %2H", tb_sram[i], CY7C1049GN_i.mem[i]);
		end
	endfunction
		...
		for(int i = 0; i < SCREEN_INIT_MEM_SIZE; i = i + 1) begin // we are checking that the actual tft_init_mem data is sent to the spi_master
			...
			if(`SCREEN_SYSTEM_PATH.tft_control_fsm_i.rom_type != 2'b10) begin // if not a delay type, we make sure that the data sent through spi is the actual data stored in the tb initialization memory
				...
				assert({`SCREEN_SYSTEM_PATH.tft_control_fsm_i.rom_type, `SCREEN_SYSTEM_PATH.spi_master_i.tnsm_data} == tft_init_mem[i])
					...
					$error("ERROR: incorrect tft_init_mem data %4X, tnsm_data %4X for entry %0d was sent to spi_master", tft_init_mem[i], `SCREEN_SYSTEM_PATH.spi_master_i.tnsm_data, i);
				...
				assert( delay_cnt >= (tft_init_mem[i][7:0]*`SCREEN_SYSTEM_PATH.tft_control_fsm_i.CLK_CYCLES_PER_MS))
					$display("PASS: correct delay %4d for entry %0d was sent to spi_master", delay_cnt, i);
				else
					$error("ERROR: incorrect delay tb %4d, tft_init_mem %4d for entry %0d was measured", delay_cnt, (tft_init_mem[i][7:0]*`SCREEN_SYSTEM_PATH.tft_control_fsm_i.CLK_CYCLES_PER_MS), i);
			end
		...
		for(int i = 0; i < TFT_SIDEBAR_COMMANDS_SIZE; i = i + 1) begin
			...
			if((i == 0) || (i == 5) || (i == 10)) // Byte transmitted is COMMAND type
				...
				assert((tft_dc == tb_dc) && (`SCREEN_SYSTEM_PATH.spi_master_i.tnsm_data == tft_sidebar[i]))
					...
					$error("ERROR: incorrect tft_sidebar data %2X, tnsm_data %2X for entry %0d was sent to spi_master", tft_sidebar[i], `SCREEN_SYSTEM_PATH.spi_master_i.tnsm_data, i);
		...
		for(int i = 0; i < TFT_ACTIVE_AREA_COMMANDS_SIZE; i = i + 1) begin
			...
			if((i == 0) || (i == 5) || (i == 10)) // Byte transmitted is COMMAND type
				...
				assert((tft_dc == tb_dc) && (`SCREEN_SYSTEM_PATH.spi_master_i.tnsm_data == tft_active_area[i]))
					...
					$error("ERROR: incorrect tft_active_area data %2X, tnsm_data %2X for entry %0d was sent to spi_master", tft_active_area[i], `SCREEN_SYSTEM_PATH.spi_master_i.tnsm_data, i);
			wait(`SCREEN_SYSTEM_PATH.tft_control_fsm_i.tnsm == 0); // as there is a handshake mechanism between spi_master and tft_control_fsm, we have to wait for tnsm to be deasserted
			@(posedge clk); // safety extra delay
		end
*/
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    uart_write_mem(8'h12, 32'h0000_0001); // initialize screen
		...
		assert(`SCREEN_SYSTEM_PATH.tft_control_fsm_i.initialization_done) $display("PASS: initialization_done flag was correctly asserted"); else $error("ERROR: initialization_done flag was not asserted on tft_control_fsm after tft initialization commands were sent");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    uart_write_mem(8'h16, 32'h0000_0000); // triggers ss_ld_sidebar
		assert(`SCREEN_SYSTEM_PATH.tft_control_fsm_i.sidebar_ongoing) $display("PASS: sidebar_ongoing flag was correctly asserted"); else $error("ERROR: sidebar_ongoing flag was not asserted on tft_control_fsm after sidebar writing commands were sent");
		#20ms;
		assert(!`SCREEN_SYSTEM_PATH.tft_control_fsm_i.sidebar_ongoing) $display("PASS: sidebar_ongoing flag was correctly deasserted"); else $error("ERROR: sidebar_ongoing flag was not deasserted on tft_control_fsm after some time");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		for(int i = 0; i < COLOR_MAP_NUM; i = i + 1) begin
			reg [15:0] color;
			color = $random;
    	uart_write_mem(8'h15, {color, i[3:0]}); // send data to the color mapping table
			...
			assert(`SCREEN_SYSTEM_PATH.color_mapping_table_i.color_map[i] == color) $display("PASS: color map index %0d was correctly written to 16'h%4h", i, color); else $error("ERROR: color map index %0d was not correctly written to 16'h%4h", i, color);
		end
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  	...
  	uart_write_mem(8'h14, 0); // trigger the screen system
		...
		assert(`SCREEN_SYSTEM_PATH.tft_control_fsm_i.busy) $display("PASS: tft_control_fsm was correctly set as busy as expected during full image loading"); else $error("ERROR: tft_control_fsm was not correctly set as busy as expected during full image loading");
		#105ms;
		assert(!`SCREEN_SYSTEM_PATH.tft_control_fsm_i.busy) $display("PASS: tft_control_fsm was correctly set as not busy as expected during full image loading"); else $error("ERROR: tft_control_fsm was not correctly set as not busy as expected during full image loading");


//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ============================================== send random objects with random values through uart commands ======================================== //
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		draw_horizontal_type(100, 100, 50, 0, 0);
		#50ms;

//    uart_write_mem(8, 32'h0000_0001); // set object type
//		repeat(10) @(posedge clk);
//    uart_write_mem(9, 32'h0000_0064); // set object width
//		repeat(10) @(posedge clk);
//    uart_write_mem(10, 32'h0000_0032); // set object height
//		repeat(10) @(posedge clk);
//    uart_write_mem(11, 32'h0000_0000); // set starting pixel address
//		repeat(10) @(posedge clk);
//    uart_write_mem(12, 32'h0000_0000); // set starting mask address
//		repeat(10) @(posedge clk);
//    uart_write_mem(13, 32'h0000_000A); // set starting x coordinate
//		repeat(10) @(posedge clk);
//    uart_write_mem(14, 32'h0000_0014); // set starting y coordinate
//		repeat(10) @(posedge clk);
//    uart_write_mem(7, 32'h0000_0050); // start drawing circuit and set object value
...
/*
  ...
  initial begin
		uart_ip_if.initialize();
    wb_master_if.wb_master_initialize();
		wait(arst_n);
		repeat(2) @(posedge clk);
//    rx = 1'b1; // TODO: need to add uart bfm
//		uart_ip_if.set_config(.baud_rate_sel(4'd7), .stop_type_sel(1'b0), .parity_type_sel(2'b00), .frame_type_sel(2'b11), .on_sel(1'b1));
		uart_ip_if.set_default_config();
		uart_ip_if.set_config_global(.baud_rate_s(14),.stop_type_s(1'b0),.parity_type_s(2'b00),.packet_size(2'b11));
		repeat(100) @(posedge clk);
		//uart_write_mem(18, 32'hFAFAFAFA); // asserts wb_slave_up_en
		repeat(100) begin
			uart_write_mem(.addr(12), .data($urandom_range(1,3))); // sets drawing object type as horizontal incremental
			uart_write_mem(.addr(13), .data($urandom_range(5,100))); // sets drawing object width as 50
			uart_write_mem(.addr(14), .data($urandom_range(5,100))); // sets drawing object height as 100
			uart_write_mem(15, 640*240+240); // sets a position to object
			uart_write_mem(16, 0); // sets starting mask
			uart_write_mem(11, 0); // starts drawing
			//uart_write_mem(12, 1); // sets drawing object type as horizontal incremental
			//uart_write_mem(13, 50); // sets drawing object width as 50
			//uart_write_mem(14, 100); // sets drawing object height as 100
			//uart_write_mem(15, 640*240+240); // sets a position to object
			//uart_write_mem(16, 0); // sets a position to object
			//uart_write_mem(11, 0); // starts drawing
			wait(`DRW_PATH.done);
			repeat(10) @(posedge clk);
		end
...
//			repeat(10) @(posedge clk);
//			wb_master_bfm.wb_master_wr_data(.wdata($random), .waddr($random & 32'h00FFFFFF));
//			repeat(10) @(posedge clk);
//			wb_master_bfm.wb_master_rd_data(.raddr($random & 32'h00FFFFFF), .rdata(tb_rdata));
//		end
  end
*/
//  // master wishbone BFM
...
/*
    ...
    .up_soft_reset(up_soft_reset),
		...
		// UART interface
		.rx(uart_ip_if.tx_test),
		.tx(uart_ip_if.rx_test),
