  ...
  always@(posedge mem_sys_clk, negedge arst_n) begin
    if(!arst_n) begin
      ...
      case(state)
        ...
        STATE_IDLE: begin
          for(integer idx = 0; idx < NUM_WRITE_BUFFERS; idx = idx + 1) begin
            if((!wr_buff_empty[idx]) && wpg_burst_ongoing[idx]) begin // buffer must be discharged - TODO: I need to check and make the performance analysis on this
              wr_buff_sel <= idx;
        ...
        STATE_WAIT_EMPTY: begin
          if(wr_buff_empty[wr_buff_sel] || wpg_burst_done[wr_buff_sel]) begin // Wait for writing buffer to be empty again - TODO: I need to check and make the performance analysis on this
            state <= STATE_IDLE;
          end
        end
