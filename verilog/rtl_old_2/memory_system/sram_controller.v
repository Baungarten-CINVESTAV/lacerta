...
assign sram_addr = (main_mem_rden) ? main_mem_rd_addr : main_mem_wr_addr; // read operation has priority
assign sram_ce_n = 1'b0; // we tie the chip enable to zero, this will be done at pcb level
assign sram_oe_n = main_mem_rden ? 1'b0 : 1'b1; // output enabled is set when we are not reading, as read operation has priority
assign sram_we_n = main_mem_rden ? 1'b1 : ~main_mem_wren; // if reading, we deactive, otherwise, we active we_n if write operation is requested
assign sram_data = main_mem_rden ? {MAIN_MEM_DATA_WIDTH{1'b0}} : main_mem_wr_data; // high impedance if reading, otherwise, we send writing data
