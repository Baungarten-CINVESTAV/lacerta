`include "defines_lacerta.v"
module dig_top (
  input wire clk, // gpio
  input wire arst_n, // gpio

  // uprocessor interface
  output wire up_soft_reset, // caravel
  ...
  // SRAM interface
  output wire [MAIN_MEM_ADDR_WIDTH-1:0] sram_addr, // gpio
  ...
  output wire sram_ce_n, // 1'b0 at pcb level
  output wire sram_oe_n, // gpio
  output wire sram_we_n, // gpio
  output wire sram_lb_n, // fpga only

  // TFT ports
  output wire sck, // gpio - sck
  output wire ss, // gpio - cs
  output wire mosi, // gpio - mosi
  // data/command send to tft screen
  output wire dc, // gpio - dc
  output wire res, // gpio - reset
  output wire blk // 1'b1 at pcb level - led
);
